...
const ADS = [
  ...
  { name: "Creative / Notion", domain: "notion.so" },
];

const PANEL_HEIGHT = 420;
const PILL_HEIGHT = 42;
const PILL_GAP = 14;
const COL_HEIGHT = ADS.length * PILL_HEIGHT + (ADS.length - 1) * PILL_GAP;
const COL_TOP = (PANEL_HEIGHT - COL_HEIGHT) / 2;
const pillCenterY = (i: number) => COL_TOP + PILL_HEIGHT / 2 + i * (PILL_HEIGHT + PILL_GAP);

function AdPill({ name, domain, muted }: { name: string; domain: string; muted?: boolean }) {
  return (
    <div
      style={{
        display: "flex",
...
function MarketerPill({ tone }: { tone: "dark" | "olive" }) {
...
export default function Slide03() {
  ...
  return (
    <div style={{ width: "100%", height: "100%", padding: "56px 80px" }}>
      ...
      >
        Marketers stop jumping between tools. Their ad stack comes to them.
      </h2>
      <p style={{ fontSize: 16, color: "#6B6863", lineHeight: 1.5, margin: "0 0 24px 0", maxWidth: 920 }}>
        We didn&apos;t replace the stack. We made Junior the surface where campaign work shows up.
      </p>
      ...
      <div style={{ display: "flex", gap: 20, height: PANEL_HEIGHT + 70 }}>
        ...
        >
          <div style={{ fontSize: 12, color: OLIVE, fontWeight: 600, letterSpacing: 0.4 }}>
            AFTER · Junior as the surface: one place to launch, monitor, optimize
          </div>
          <div style={{ position: "relative", height: PANEL_HEIGHT, marginTop: 14 }}>
            <svg
              width="100%"
      ...
      >
        <span style={{ color: OLIVE, fontWeight: 700, fontSize: 13 }}>Purpose-built for Ads</span>
        <span style={{ color: MUTE }}>|</span>
        <span>Drafts creatives. Watches spend. Surfaces winners. Routes next actions.</span>
      </div>
    </div>
  );
}
