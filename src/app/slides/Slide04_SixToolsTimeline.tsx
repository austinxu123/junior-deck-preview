...
const STEPS = [
  { name: "Clay", domain: "clay.com", label: "Lead enriched" },
  { name: "HubSpot", domain: "hubspot.com", label: "Scored & routed" },
  { name: "Salesforce", domain: "salesforce.com", label: "Opp created" },
  { name: "Gong", domain: "gong.io", label: "Call insights" },
  { name: "PandaDoc", domain: "pandadoc.com", label: "Proposal / contract" },
  { name: "Slack", domain: "slack.com", label: "Closed-won posted" },
];

// Visual layout coords (viewBox space).
const VB_W = 1120;
const VB_H = 360;
const JUNIOR_X = VB_W / 2;
const JUNIOR_Y = 60;
const ROW_Y = 250; // y of tool card centers (where dotted curves land)
const CARD_W = 150;
const CARD_GAP = 16;
  ...
  return (
    <div style={{ width: "100%", height: "100%", padding: "56px 80px" }}>
      ...
      <div style={{ position: "relative", width: "100%" }}>
        {/* SVG layer for dotted curves + horizontal arrows */}
        <svg
          width="100%"
          ...
          aria-hidden="true"
        >
          {/* Junior badge background (rendered in DOM below; here just markers) */}
          {/* Dotted curves: Junior → each tool */}
          {STEPS.map((_, i) => {
            const x2 = cardCenterX(i);
            const y2 = ROW_Y - 50; // top of card
            ...
            return (
              <g key={`curve-${i}`}>
                <path
                  ...
                  strokeLinecap="round"
                />
                {/* endpoint dot on the card */}
                <circle cx={x2} cy={y2} r={3} fill={OLIVE} />
              </g>
            );
          })}

          {/* Horizontal arrows between tool cards */}
        ...
        {/* Overlaid HTML: Junior badge + tool cards */}
      ...
      >
        <span style={{ color: OLIVE, fontWeight: 700, fontSize: 13 }}>Purpose-built for Sales</span>
        <span style={{ color: MUTE }}>|</span>
        <span>Context-aware. Route-ready. Built on systems your team already trusts.</span>
      </div>
    </div>
  );
}
